    ...
    protected function blockListComplete(array $block)
    {
        ...
        for ($iterator_element = 0; $iterator_element < $count_element; $iterator_element++) {
            ...
            for ($iterator_text = 0; $iterator_text < $count_text; $iterator_text++) {
                ...
                if ('[ ] ' === $begin_line) {
                    $block['element']['text'][$iterator_element]['text'][$iterator_text] = '<input type="checkbox" class="task-list-item-checkbox" disabled /> '.
                        substr(trim($block['element']['text'][$iterator_element]['text'][$iterator_text]), 4);
                    $block['element']['text'][$iterator_element]['attributes'] = ['class' => 'parsedown-task-list parsedown-task-list-open task-list-item'];
                } elseif ('[x] ' === $begin_line) {
                    $block['element']['text'][$iterator_element]['text'][$iterator_text] = '<input type="checkbox" class="task-list-item-checkbox" checked disabled /> '.
                        substr(trim($block['element']['text'][$iterator_element]['text'][$iterator_text]), 4);
                    $block['element']['text'][$iterator_element]['attributes'] = ['class' => 'parsedown-task-list parsedown-task-list-close task-list-item'];
                }
            }
        }
